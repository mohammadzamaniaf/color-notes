import 'package:flutter/material.dart';
import 'package:hello_world/models/note_color.dart';

const colors = <NoteColor>[
  NoteColor(headerColor: Colors.yellow, bodyColor: Colors.yellowAccent),
  NoteColor(headerColor: Colors.pink, bodyColor: Colors.pinkAccent),
  NoteColor(headerColor: Colors.green, bodyColor: Colors.lightGreen),
  NoteColor(headerColor: Colors.lime, bodyColor: Colors.limeAccent),
  NoteColor(headerColor: Colors.deepPurple, bodyColor: Colors.purple),
  NoteColor(headerColor: Colors.red, bodyColor: Colors.redAccent),
  NoteColor(headerColor: Colors.blue, bodyColor: Colors.lightBlue),
  NoteColor(headerColor: Colors.cyan, bodyColor: Colors.cyanAccent),
  NoteColor(headerColor: Colors.grey, bodyColor: Colors.blueGrey),
  NoteColor(headerColor: Colors.teal, bodyColor: Colors.tealAccent),
  NoteColor(headerColor: Colors.deepOrange, bodyColor: Colors.orange),
];
